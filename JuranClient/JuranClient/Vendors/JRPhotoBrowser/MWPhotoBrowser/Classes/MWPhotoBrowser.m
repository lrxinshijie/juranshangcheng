...
- (void)viewDidLoad {
    ...
    */
    ...
    _bottomView = [[UIView alloc] initWithFrame:[self frameForBottomViewAtOrientation:self.interfaceOrientation]];
...
- (void)layoutVisiblePages {
     ...
     */
    _bottomView.frame = [self frameForBottomViewAtOrientation:self.interfaceOrientation];
...
- (id<MWPhoto>)currentPhoto{
    return [self photoAtIndex:self.currentIndex];
}
...
- (CGSize)contentSizeForPagingScrollView {
    ...
    CGRect bounds = _pagingScrollView.bounds;
    return CGSizeMake(bounds.size.width * [self numberOfPhotos]+1, bounds.size.height);
}
...
- (CGRect)frameForBottomViewAtOrientation:(UIInterfaceOrientation)orientation{
    CGFloat height = 0;
    if (self.bottomView) {
...
- (CGRect)frameForCaptionView:(MWCaptionView *)captionView atIndex:(NSUInteger)index {
    ...
    CGRect captionFrame = CGRectMake(pageFrame.origin.x,
                                     pageFrame.size.height - captionSize.height - (0),//_toolbar.superview?_toolbar.frame.size.height:
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if (!_scrollNotHiddenControlEnable) {
...
//        _toolbar.frame = CGRectOffset([self frameForToolbarAtOrientation:self.interfaceOrientation], 0, animatonOffset);
        _bottomView.frame = CGRectOffset([self frameForBottomViewAtOrientation:self.interfaceOrientation], 0, animatonOffset);
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        if (slideAndFade) {
            _bottomView.frame = [self frameForBottomViewAtOrientation:self.interfaceOrientation];
            if (hidden) {
                _bottomView.frame = CGRectOffset(_bottomView.frame, 0, animatonOffset);
            }
        }
...
- (void)hideControlsAfterDelay {
	if (![self areControlsHidden] && !_isNotAutoHiddenControl) {
        [self cancelControlHiding];
...
- (BOOL)areControlsHidden { return (_bottomView.alpha == 0); }
