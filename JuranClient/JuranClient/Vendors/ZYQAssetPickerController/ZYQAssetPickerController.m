...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            ...
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld Videos", nil), (long)_numberOfVideos];
        else
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld Photos, %ld Videos", nil), (long)_numberOfPhotos, (long)_numberOfVideos];
